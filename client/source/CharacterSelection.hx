	...
	function changeSelection(change:Int = 0)
	{
		if (nospam == false)
		{
			...
			funkers.forEach(function(stuff:FlxSprite)
			{
				...
				if (curSelected == 0 && !ticktockclock)
					FlxTween.tween(stuff, {x: stuff.x + 500}, 0.2, {ease: FlxEase.quadInOut, onComplete: function(twn:FlxTween)
					{
						nospam = false;
					}});
				if (curSelected == controlsStrings.length && ticktockclock)
					FlxTween.tween(stuff, {x: stuff.x + 500 * controlsStrings.length}, 0.2, {ease: FlxEase.quadInOut, onComplete: function(twn:FlxTween)
					{
						nospam = false;
					}});
				else
				{
					if (change == -1 && !ticktockclock)
						FlxTween.tween(stuff, {x: stuff.x + 500}, 0.2, {ease: FlxEase.quadInOut, onComplete: function(twn:FlxTween)
						{
							nospam = false;
						}});
					else if (change == 1 && !ticktockclock)
						FlxTween.tween(stuff, {x: stuff.x - 500}, 0.2, {ease: FlxEase.quadInOut, onComplete: function(twn:FlxTween)
						{
							nospam = false;
						}});

...
typedef NDT =
{ // NDT means Nessecary Data Types btw!
	var message:String;
	var chatHist:String;
...
class ChatState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var coly = new Client('wss://ws.sancopublic.com');
		...
		if (FlxG.save.data.username != null)
			...
			username = "guest" + FlxG.random.int(0, 9999);

		UI_box = new FlxUITabMenu(null, [{name: "tab1", label: 'MOTD'}, {name: "tab2", label: 'Rules'},], true);
		...
		coly.joinOrCreate("chat", [], Stuff, function(err, room)
		{
			...
			room.onMessage("message", function(message)
			{
				...
				chatText.applyMarkup(chatText.text, [new FlxTextFormatMarkerPair(new FlxTextFormat(FlxColor.GREEN), "[G]")]);
				chatText.y -= 16;
	...
	public function changeUsername()
	{
		...
		if (usnbox.text != "")
		{
			...
			usnbox.text = "";
			if (connected)
				rooms.send("userdata", {usname: username});
		}
	}
}

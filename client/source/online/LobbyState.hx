...
class LobbyState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var UI_box = new FlxUITabMenu(null, [{name: "tab1", label: 'Player'}, {name: "tab2", label: 'Info'},], true);
		...
		var namebtn = new FlxButton(usnbox.x, usnbox.y + 15, "Change Name", function()
		{
			if (usnbox.text != "")
			{
				if (ConnectingState.conmode == "host")
					ConnectingState.p1name = usnbox.text;
				else
					ConnectingState.p2name = usnbox.text;
				usnbox.text = "";
				usnbox.caretIndex = 0;
				if (ConnectingState.conmode == "host")
					rooms.send("chatHist", {name: ConnectingState.p1name});
				else
					rooms.send("chatHist", {name: ConnectingState.p2name});
			}
		});

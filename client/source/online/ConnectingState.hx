...
class ConnectingState extends MusicBeatState
{
	...
	var nmsongs:Array<String> = [
		'tutorial', 'test', 'bopeebo', 'fresh', 'dadbattle', 'spookeez', 'south', 'monster', 'pico', 'philly', 'blammed', 'satin-Panties', 'high', 'milf',
	...
	public function new(state:String, type:String, ?code:String)
	{
		...
		coly = new Client('wss://ws.sancopublic.com');
		Note.single = false;
		switch (state)
		{
			case 'battle':
				if (type == "host")
				{
					...
					if (FlxG.save.data.username != null)
						p1name = FlxG.save.data.username;
					else
						p1name = "guest" + FlxG.random.int(0, 9999);
					p2name = "";
					...
					coly.create("battle", [], Stuff, function(err, room)
					{
						...
						try
						{
							...
							{
								if (LobbyState.code == message.iden)
									PlayStateOnline.onlinemodetext.text = "Player Found! Starting...";
							...
							room.onMessage("misc", (message) ->
							{
								if (message.p1)
								{
									LobbyState.playertxt.members[0].applyMarkup("/r/Ready/r/",
										[new FlxTextFormatMarkerPair(new FlxTextFormat(FlxColor.GREEN), "/r/")]);
								}
								else
								{
									LobbyState.playertxt.members[0].applyMarkup("/r/Not Ready/r/",
										[new FlxTextFormatMarkerPair(new FlxTextFormat(FlxColor.RED), "/r/")]);
								}
								if (message.p2)
								{
									LobbyState.playertxt.members[1].applyMarkup("/r/Ready/r/",
										[new FlxTextFormatMarkerPair(new FlxTextFormat(FlxColor.GREEN), "/r/")]);
								}
								else
								{
									LobbyState.playertxt.members[1].applyMarkup("/r/Not Ready/r/",
										[new FlxTextFormatMarkerPair(new FlxTextFormat(FlxColor.RED), "/r/")]);
								}
							});
							...
							room.onMessage('userleft', function(message)
							{
								...
								if (PlayStateOnline.startedMatch)
									PlayStateOnline.leftText.text = "User left the game.";
								if (!PlayStateOnline.startedMatch)
									p2name = '';
								if (!PlayStateOnline.startedMatch)
									LobbyState.playertxt.members[1].applyMarkup("/r/Not Ready/r/",
						...
						catch (e:Any)
						{
							...
							if (FlxG.random.bool(0.1))
								PlayStateOnline.onlinemodetext.text = "bitch git gud internet";
							else
								PlayStateOnline.onlinemodetext.text = "Not connected to the server.";
				...
				else if (type == "join")
				{
					conmode = 'join';
					if (FlxG.save.data.username != null)
						p2name = FlxG.save.data.username;
					else
						p2name = "guest" + FlxG.random.int(0, 9999);
					try
					{
						coly.join("battle", [], Stuff, function(err, room)
						{
							...
							room.onMessage("message", function(message)
							{
								...
								if (!nmsongs.contains(message.song))
								{
									...
									PlayStateOnline.modinst = new Sound(new URLRequest('https://ws.sancopublic.com/songs/$sng/Inst.ogg'));
									loadprog = new Alphabet(loadprog.x, loadprog.y, "Loading Voices...");
									PlayStateOnline.modvoices = new Sound(new URLRequest('https://ws.sancopublic.com/songs/$sng/Voices.ogg'));
									loadprog = new Alphabet(loadprog.x, loadprog.y, "Loading Chart...");

									var http = new haxe.Http('https://ws.sancopublic.com/songs/$sng/chart$modif.json');
